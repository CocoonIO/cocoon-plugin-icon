...
module.exports = function(context) {
    if (context.opts.cordova.platforms.indexOf('osx') <= -1)
        return;

    // Get the osx project name
    var osx_path = fs.readdirSync(path.join(context.opts.projectRoot, "platforms", "osx"));
    var xcodeproj_path = osx_path.filter(function(item) { 
    ...
    var icons_path = path.join(context.opts.projectRoot, 'platforms', 'osx', project_name, 'Images.xcassets', 'AppIcon.appiconset');
    ...
    icons.forEach(function(icon) {
        try {
            ...
            var src_image_file = path.join(context.opts.projectRoot, '..', 'icons', 'osx.png');
